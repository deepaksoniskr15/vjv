...
public class PermissionController {
	...
	public ApiResponseDto addPatient(@RequestBody Permission permission) {
		...
		if (permission.getId() != null) {
			apiResponseDtoBuilder.withMessage("Request Send To Patient Successfully").withStatus(HttpStatus.OK)
					.withData(permission);
		} else {
	...
	@RequestMapping(value = "/permission/get/requesterAddress/{walletAddress}", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.GET)
	public ApiResponseDto getRequestListByRequester(@PathVariable String walletAddress) {
		ApiResponseDtoBuilder apiResponseDtoBuilder = new ApiResponseDtoBuilder();
		List<Permission> list = permissionRepository.findByRequesterAddress(walletAddress);
		apiResponseDtoBuilder.withMessage("success").withStatus(HttpStatus.OK).withData(list);
		return apiResponseDtoBuilder.build();
	}

	@RequestMapping(value = "/permission/get/patientAddress/{walletAddress}", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.GET)
	public ApiResponseDto getRequestListByPatient(@PathVariable String walletAddress) {
		ApiResponseDtoBuilder apiResponseDtoBuilder = new ApiResponseDtoBuilder();
		List<Permission> list = permissionRepository.findByPatientAddress(walletAddress);
		apiResponseDtoBuilder.withMessage("success").withStatus(HttpStatus.OK).withData(list);
		return apiResponseDtoBuilder.build();
	}
}
